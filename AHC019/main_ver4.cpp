/*
    ...
    2 組の共通部分で連結な部分をすべて探索してブロックを置く。
    その後、空いているところを ver1 の方法を使って立方体で埋める
*/
...
void block_bfs(int D, vector<vector<char>> &f, vector<vector<char>> &r, int &n, vector<i64> &b, vector<vector<vector<bool>>> &visited){
            ...
            for(int z = 0; z < D; z++){
                ...
                while(!Q.empty()){
                    ...
                    for(int i = 0; i < 6; i++){
                        ...
                        visited.at(nv.x).at(nv.y).at(nv.z) = true;
                    }
                }
            }
        }
...
void common_block_bfs(int D, vector<vector<vector<char>>> &f, vector<vector<vector<char>>> &r, int &n, vector<vector<i64>> &b, vector<vector<vector<bool>>> &visited){
    // vector<vector<vector<bool>>> visited(D, vector<vector<bool>>(D, vector<bool>(D, false)));
    queue<Point> Q;
    ...
    for(int x = 0; x < D; x++){
        for(int y = 0; y < D; y++){
            for(int z = 0; z < D; z++){
                ...
                if(f.at(0).at(z).at(x) == '0' || r.at(0).at(z).at(y) == '0'){
                    continue;
                }
                if(f.at(1).at(z).at(x) == '0' || r.at(1).at(z).at(y) == '0'){
                    continue;
                }
                ...
                while(!Q.empty()){
                    Point v = Q.front();
                    Q.pop();
                    b.at(0).at(ret_b_index(D, v.x, v.y, v.z)) = n;
                    b.at(1).at(ret_b_index(D, v.x, v.y, v.z)) = n;
                    ...
                    for(int i = 0; i < 6; i++){
                        ...
                        if(f.at(0).at(nv.z).at(nv.x) == '0' || r.at(0).at(nv.z).at(nv.y) == '0'){
                            continue;
                        }
                        if(f.at(1).at(nv.z).at(nv.x) == '0' || r.at(1).at(nv.z).at(nv.y) == '0'){
                            continue;
                        }
...
int main() {
    ...
    int n = 0;
    vector<vector<vector<bool>>> visited(D, vector<vector<bool>>(D, vector<bool>(D, false)));
    common_block_bfs(D, f, r, n, b, visited);

    vector<i64> n_cor(2, n);
    for(int i = 0; i < 2; i++){
        for(int x = 0; x < D; x++){
            for(int y = 0; y < D; y++){
                for(int z = 0; z < D; z++){
                    ...
                    if(f.at(i).at(z).at(x) == '1' && r.at(i).at(z).at(y) == '1'){
                        n_cor.at(i) += 1;
                        b.at(i).at(ret_b_index(D, x, y, z)) = n_cor.at(i);
                    }
                }
            }
        }
    }
    int ret_n = max(n_cor.at(0), n_cor.at(1));
