...
void naive_random_solution(int N, vector<vector<int>> &b, vector<Point> &op1, vector<Point> &op2, int &K){
    // K = 0;
    while(K < LOOP_MAX){
        bool out = true;
        for(int i = 0; i < N-1; i++){
        ...
        for(int i = 0; i < N-1; i++){
            for(int j = 0; j <= i; j++){
                ...
                if(!is_correct_order(b, now_ball, next_ball1) && !is_correct_order(b, now_ball, next_ball2)){
                    ...
                    if(b.at(next_x1).at(next_y1) < b.at(next_x2).at(next_y2)){
                        swap_balls(b, now_ball, next_ball1, op1, op2);
                        K++;
                        out = false;
                        continue;
...
int main() {
    ...
    while(sw.GetTimeMS() < 1950){
        vector<Point> op1, op2;
        vector<vector<int>> now_ans = b;
        int K = 0;
        naive_random_solution(N, now_ans, op1, op2, K);
