...
// 操作回数を返す
int naive_solution(int N, vector<vector<int>> &b, vector<Point> &op1, vector<Point> &op2){
    int K = 0;
    while(K < LOOP_MAX){
        bool out = true;
        for(int i = 0; i < N-1; i++){
...
int main() {
    ...
    int K = naive_solution(N, ans, op1, op2);

    output_func(K, op1, op2);
