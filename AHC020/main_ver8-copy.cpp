...
int main() {
    ...
    for(int i = 0; i < input_M; i++){
        auto [weight, idx] = edge_weight.at(i);
        i64 v1 = input_u.at(idx);
        i64 v2 = input_v.at(idx);
        if(uf.same(v1, v2)){
            continue;
        }
        ...
        dig.at(v1)++;
        dig.at(v2)++;
        used_edge.at(v1).emplace_back(Edge(v1, v2, weight, idx));
        used_edge.at(v2).emplace_back(Edge(v2, v1, weight, idx));
        output_B.at(idx) = 1;
        uf.merge(v1, v2);
        if(need_housou.at(v1)){
            if(!visited.at(v1)){
                visited.at(v1) = true;
                need_housou_cnt++;
            }
        }
        if(need_housou.at(v2)){
            if(!visited.at(v2)){
                visited.at(v2) = true;
                need_housou_cnt++;
            }
    ...
    */
    ...
    // vector<bool> is_used_vertex(input_N, false);
    for(int i = 0; i < input_N; i++){
        if(need_housou.at(i)){
            // do nothing
        }
            ...
            if(dig.at(i) == 1){
                ...
                dig.at(now_edge.v2)--;
                i64 pre_v = i;
                while((dig.at(now_edge.v2)) == 1 && !(need_housou.at(now_edge.v2)) && (now_edge.v2 != 0)){
                    dig.at(now_edge.v2)--;
    ...
    for(int i = 0; i < input_N; i++){
        ...
        if(dig.at(i) == 1){
            ...
            output_B.at(now_edge.index) = 0;
            dig.at(i)--;
            while((dig.at(now_edge.v2)) == 2 && !(need_housou.at(now_edge.v2)) && (now_edge.v2 != 0)){
                Edge nxt_edge;
                for(Edge ed : used_edge.at(now_edge.v2)){
                    if(ed.v2 != i){
                        nxt_edge = ed;
