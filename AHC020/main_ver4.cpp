...
int main() {
    ...
    vector<bool> done(input_N, false);
    vector<i64> dist(input_N, I64_INF);
    dist.at(0) = 0;
    visited.at(0) = true;
    for(int i = 0; i < input_N; i++){
        pq.emplace(dist.at(i), i);
    }
    ...
    while(!pq.empty()){
        ...
        for(Edge nxt : G.at(v)){
            i64 nv = nxt.v2;
            if(dist.at(nv) > dist.at(v) + nxt.w){
                dist.at(nv) = dist.at(v) + nxt.w;

                debugV(nv);
                debugVL(dist.at(nv));

                pq.emplace(dist.at(nv), nv);
                visited.at(nv) = true;
    ...
    for(int i = 0; i < input_N; i++){
        if(need_housou.at(i)){
            output_P.at(i) = (i64)(sqrt(output_P.at(i)) + 0.5);
        }
        else{
            output_P.at(i) = 0;
        }
