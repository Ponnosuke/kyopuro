/*
    ...
    ver25 から継承、次数の大きい頂点からターゲットにして、そこから全頂点への経路を求めてそれによって評価
    始点となる頂点の数を target_vertex_cnt = 1 で管理
...
const double LIMIT_TIME = 5900.0;
...
int main() {
    ...
    */
    ...
    vector<unordered_set<int>> kouji(D+1);  // kouji[d] := d(1-indexed) 日に工事をする辺の index (0-indexed) の unordered_set
    for(int i = 0; i < M; i++){
        kouji[r[i]].insert(i);
    }
    ...
    while(!stop){
        ...
        vector<i64> prev_day_prev_d_mono, prev_day_next_d_mono;
        vector<i64> next_day_prev_d_mono, next_day_next_d_mono;
        ...
        prev_day_prev_d_mono = get_dk_mono(N, M, max_deg_v, prev_day_prev_G);
        ...
        next_day_prev_d_mono = get_dk_mono(N, M, max_deg_v, next_day_prev_G);
        ...
        prev_day_next_d_mono = get_dk_mono(N, M, max_deg_v, prev_day_next_G);
        ...
        next_day_next_d_mono = get_dk_mono(N, M, max_deg_v, next_day_next_G);
        ...
        judge_value += ( get_sum_d_mono_diff(N, prev_day_prev_d_mono, prev_day_next_d_mono) + get_sum_d_mono_diff(N, next_day_prev_d_mono, next_day_next_d_mono) );
