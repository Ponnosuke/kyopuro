/*
    ダイクストラ法を使って不満度を一日ずつ愚直に計算していくプログラム
    答えは合ってるっぽいけど計算にめちゃくちゃ時間かかる (13秒位)
*/
...
int main() {
    ...
    for(int day = 1; day <= D; day++){
        ...
        for(int i = 0; i < M; i++){
            ...
            today_G.at(u.at(i)).push_back(Edge(u.at(i), v.at(i), w.at(i), i));
            today_G.at(v.at(i)).push_back(Edge(v.at(i), u.at(i), w.at(i), i));        
        }

        vector<vector<i64>> d_k = get_dk(N, M, today_G);
