/*
    ...
    没。時間がかかりすぎて 3 回とかしか回せない

    xorshift で乱数生成

    ver21から継承、完全ランダム生成探索。次数の大きい頂点からターゲットにして、そこから全頂点への経路を求めてそれによって評価
    始点となる頂点の数を target_vertex_cnt = N/5 で管理

    int route_select_value を実装して、これの値によって、辺を移すか交換するかを選ぶ
    route_select_flag = (M / D) / K > route_select_flag_level = 0.7 のとき、route_select_value = rnd(), 
    そうじゃないとき route_select_value = 100
*/
...
vector<i64> get_dk_mono(int N, int M, int v_k, vector<vector<Edge>> &G){
    ...
    dk_mono[v_k] = 0;
    vector<bool> done(N, false);
    pq.emplace(make_pair(dk_mono[v_k], v_k));
    while(!pq.empty()){
        auto [d, v] = pq.top();
        pq.pop();
...
/*
    today のグラフを得る。today は 1-indexed
*/
vector<vector<Edge>> get_today_G(int N, int M, int today, vector<set<int>> &kouji, vector<int> &u, vector<int> &v, vector<i64> &w){
    vector<vector<Edge>> ret_G(N);
    for(int i = 0; i < M; i++){
        if(kouji[today].count(i)){
            continue;
        }

        int tu = u[i], tv = v[i];
        i64 tw = w[i];
        ret_G[tu].emplace_back(Edge(tu, tv, tw, i));
        ret_G[tv].emplace_back(Edge(tv, tu, tw, i));
    }
...
int main() {
    ...
    */
    ...
    bool stop = false;
    // i64 debugCnt = 0;
    int target_vertex_cnt = N/5;

    vector<vector<vector<i64>>> prev_d(D, vector<vector<i64>>(target_vertex_cnt, vector<i64>(N, INF9)));
    // prev_d[day][target_vertex_cnt][to_vertex] := day が 0-indexed なのに注意
    for(int day = 1; day <= D; day++){
        vector<vector<Edge>> today_G = get_today_G(N, M, day, kouji, u, v, w);
        for(int index = 0; index < target_vertex_cnt; index++){
            int now_v = deg[index].second;
            prev_d[day-1][index] = get_dk_mono(N, M, now_v, today_G);
        }
    }
    ...
    /*
    for(int i = 0; i < D; i++){
        for(int j = 0; j < target_vertex_cnt; j++){
            for(int k = 0; k < N; k++){
                if(prev_d[i][j][k] != INF9){
                    debugV(i);
                    debugV(j);
                    debugV(k);
                    debugVL(prev_d[i][j][k]);
                }
            }
        }
    }
    */
    i64 debug_cnt = 0;
    vector<pair<double, int>> debug_out;
    vector<i64> debug_judge_value;
    ...
    while(true){
        ...
        vector<int> next_r(M, 1);
        vector<int> next_dayCnt(D, 0);  // 0-indexed
        for(int i = 0; i < M; i++){
            int day = 1;
            do{
                day = xRnd(1, D);
            }while(next_dayCnt[day-1] >= K);
        ...
        // assert(r != next_r);
        // assert(dayCnt != next_dayCnt);

        vector<set<int>> next_kouji(D+1);  // next_kouji[d] := d(1-indexed) 日に工事をする辺の index (0-indexed) の set
        for(int i = 0; i < M; i++){
            next_kouji[next_r[i]].insert(i);
        }

        // assert(kouji != next_kouji);
        // debugWL(PASS);

        vector<vector<vector<i64>>> next_d(D, vector<vector<i64>>(target_vertex_cnt, vector<i64>(N, INF9)));
        for(int day = 1; day <= D; day++){
            vector<vector<Edge>> next_today_G = get_today_G(N, M, day, next_kouji, u, v, w);
            for(int index = 0; index < target_vertex_cnt; index++){
                int now_v = deg[index].second;
                next_d[day-1][index] = get_dk_mono(N, M, now_v, next_today_G);
            }
        }
        ...
        for(int day = 0; day < D; day++){
            for(int i = 0; i < target_vertex_cnt; i++){
                judge_value += get_sum_d_mono_diff(N, prev_d[day][i], next_d[day][i]);
            }
        }
        ...
        if(judge_value < 0){
            ...
            debug_out.emplace_back(make_pair(sw.GetTimeMS(), 1));
        }
        else{
            debug_out.emplace_back(make_pair(sw.GetTimeMS(), 0));
        }
        debug_judge_value.emplace_back(judge_value);
    }
    ...
    double route_select_flag = ((double)M / (double)D) / (double)K;
    double route_select_flag_level = 0.7;
    int route_select_value;
    if(route_select_flag > route_select_flag_level){
        route_select_value = xRnd(1, 100);
    }
    else{
        route_select_value = 100;
    }
    ...
    */
    ...
    for(int i = 0; i < debug_out.size(); i++){
        debugV(debug_judge_value[i]);
        debugV(debug_out[i].first);
        if(debug_out[i].second == 1){
            debugWL(change);
        }
        else{
            debugWL(not change);
