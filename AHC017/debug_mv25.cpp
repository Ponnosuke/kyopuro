/*
    ...
    ver24 から継承、次数の大きい頂点からターゲットにして、そこから全頂点への経路を求めてそれによって評価
    ...
    while(!stop){
        ...
        if(route_select_value > route_select_level){
            ...
            while(prev_day == next_day || dayCnt[next_day - 1] + 1 > K){
                next_day = xRnd(1, D);
                // next_day = myRnd64(1, D);

                debug_out_cnt++;
                if(debug_out_cnt >= 100){
                    for(int i = 0; i < dayCnt.size(); i++){
                        if(dayCnt[i] <= (K-1)){
                            
                            next_day = i+1;

                            assert(dayCnt[next_day - 1] < K);
