...
int main() {
    ...
    for(int i = 2; i < MAX; i++)  isPrime.set(i, 1);  // 右から 0 始まりで i 番目のビットを 1 に

    // 素数ふるい
    for(int i = 2; i < MAX; i++){
        if(isPrime.test(i)){
            for(int j = i * 2; j < MAX; j += i){
                isPrime.reset(j);  // 右から 0 始まりで j 番目のビットを 0 に
            }
        }
    }
    ...
    for(int i = 0; i < MAX; i++){
        ...
        if(isPrime.test(i) && isPrime.test((i+1)/2))  like2017.at(i) = 1;
